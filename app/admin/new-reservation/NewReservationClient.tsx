    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-white">{isEdit ? 'Editar Reserva' : 'Nueva Reserva'}</h1>
          <Link href="/admin" className="text-zinc-400 hover:text-white text-sm transition">← Volver</Link>
        </div>

        {errors.form && (
          <div className="bg-red-950/50 border border-red-800/50 rounded-xl p-3 mb-4 text-red-400 text-sm">{errors.form}</div>
        )}
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          ...
          <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wide mb-4">Datos del Cliente</h2>
          ...
          {/* Reservation details */}
          <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wide mb-4">Detalles de la Reserva</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* Table selector */}
          <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wide mb-1">Mesa(s)</h2>
            <p className="text-zinc-600 text-xs mb-3">Selecciona fecha + hora para ver disponibilidad.</p>
            ...
            {(!date || !time) ? (
              <p className="text-zinc-600 text-sm">Elige fecha y turno primero.</p>
            ) : loadingTables ? (
              <p className="text-zinc-500 text-sm">Verificando disponibilidad…</p>
            ) : (
              <>
                ...
                {selectedTables.length > 0 && (
                  <p className="mt-2 text-xs text-zinc-500">
                    Seleccionadas: <span className="text-zinc-300 font-mono">{selectedTables.join(', ')}</span>
                  </p>
                )}
              </>
            )}
          ...
          {/* Notes */}
          <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wide mb-3">Notas Internas</h2>
            <textarea
              value={notes}
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <Link href="/admin" className="px-5 py-2.5 rounded-lg border border-zinc-700 text-zinc-400 hover:text-white text-sm transition">
              Cancelar
            </Link>
            {isEdit && (
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-zinc-500 text-xs uppercase tracking-wide font-medium">{label}</label>
      {children}
...
function inputCls(hasError: boolean) {
  return `w-full bg-zinc-800 border ${hasError ? 'border-red-700' : 'border-zinc-700'} rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-orange-500 placeholder-zinc-600`
}
